...
class ServiceVSTRM(ServiceBase):
    ...
    def h264_nal_encapsulate(s, is_idr, vstrm, nals):
    ...
    def update(s, packet):
        ...
        if s.is_streaming and h.frame_end:
            s.h264_nal_encapsulate(is_idr, s.frame, s.nals)
            nals_string = s.nals.tostring()
            s.decoder.display_frame(nals_string)
            s.nals.zero()
